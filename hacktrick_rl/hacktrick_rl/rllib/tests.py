...
from hacktrick_rl.rllib.utils import softmax, get_required_arguments, iterable_equal
...
class RllibEnvTest(unittest.TestCase):
    ...
    def setUp(self):
        self.params = copy.deepcopy(HacktrickMultiAgent.DEFAULT_CONFIG)
    ...
    def _test_bc_schedule(self, bc_schedule, expected_bc_factors):
        ...
        env = HacktrickMultiAgent.from_config(self.params)
        actual_bc_factors = []
    ...
    def test_env_creation(self):
        ...
        for sched in invalid_schedules:
            ...
            self.assertRaises(AssertionError, HacktrickMultiAgent.from_config, self.params)
    ...
    def test_bc_annealing(self):
        # Test no annealing
        self._test_bc_schedule(HacktrickMultiAgent.self_play_bc_schedule, [0.0]*len(self.timesteps))
